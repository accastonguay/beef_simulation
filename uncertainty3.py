    ...
    if feed_option in ['v1', 'v2']:
        ...
        for l in ['grass_grain']:
            ...
            meat = (np.nansum(biomass_dom * feed_energy[foddercrop_list].iloc[0].values[None, :], axis=1) + (
                        feats['grain_grassBM'].values * feats.merge(grass_energy, how='left', left_on=['region', 'glps'], right_on=['region', 'glps'])['ME'].values)) * \
            ...
            cstock2 = np.take_along_axis(feats[[lu + '_cstock' for lu in grass_cols]].values, grasslu[:, None], axis=1).flatten()

            # 0 C stock for grain. For grass, take cstock from the best grass land use and multiply by the fraction of area
            feats[l + '_cstock'] = cstock2 * (grass_area/grassarea2)
        ...
        for l in ['stover_grass']:
            ...
            feats[l + '_meat'] = meat_stover + np.take_along_axis(feats[[lu + '_meat' for lu in grass_cols]].values,
                       grasslu[:, None], axis=1).flatten()
        ...
        for l in ['stover_grain']:
            ...
            stover_fraction = feats[['region']].merge(feed_compo[['region', 'stover']], how = 'left')['stover'].values
            stover_max = feats['stover_grain_grainBM'] * (stover_fraction / (1-stover_fraction))
            ...
            # Meat production (t) = sum across feeds (Domestic biomass (t) x ME in feed (MJ/kd DM)) x ME to beef conversion ratio * dressing (%)
            meat = (np.nansum(biomass_dom * feed_energy[foddercrop_list].iloc[0].values[None, :], axis=1) + feats['stover_energy'].values * stov_adjustment) * \
                   feats[['group', 'glps']].merge(subset_table, how='left', left_on=['group', 'glps'],
                                                  right_on=['group', 'glps'])[beef_yield].values * dressing
            # meat = (np.nansum(biomass_dom * feed_energy[foddercrop_list].iloc[0].values[None, :], axis=1) + (
...
def main(export_folder ='.', optimisation_method= 'weighted_sum', lam = 0.5, demand_scenario = 'Demand',
    ...
    opp_soc_curr = grid.loc[grid.changed == 0, 'aff_cost'].sum()
    ...
    newcols = [c for c in cols + ["compensation", 'aff_cost','opp_aff','opp_soc', 'grass_BM', 'grain_BM', 'stover_BM']]
    # for c in newcols:
    #     logger.info(c)
    ...
    country_data = grid[['ADM0_A3', 'total_cost','total_emission', 'production', 'biomass', 'beef_area', "compensation",
                         'aff_cost','opp_aff', 'opp_soc', 'grass_BM', 'grain_BM', 'stover_BM']].groupby('ADM0_A3', as_index=False).sum()
    ...
    grid[['geometry'] + newcols].to_file(export_folder + '/' + constraint + "_" + str(crop_yield) + '_' + beef_yield + '_' + str(lam) + '_' + dem + "_" + feed_option + "_" + aff_scenario + ".gpkg", driver="GPKG")
