...
def scoring(feats, optimisation_method, crop_yield, lam, beef_yield, aff_scenario, logger, feed_option):
    """
    ...
    if feed_option in ['v1', 'v2']:
        landuses = grass_cols + ['grass_grain', 'grass_stover', 'grain_stover']  # landuses to include in the simulation

        for l in grass_cols:

            # Calculate biomass consumed (ton) = (grazed biomass (t/ha) * area (ha))
            # biomass_consumed = feats[l].values * feats['suitable_area'].values
            feats['grass_grassBM'] = feats[l].values * feats['available_area'].values
            ...
            feats[l + '_meth'] = feats['grass_grassBM'] * \
            ...
            feats[l + '_manure'] = feats['grass_grassBM'] * feats[['group']].merge(grouped_ME, how='left')['nitrous'].values
            ...
            feats[l + '_cstock'] = 0.475 * (feats['grass_grassBM'] / grazing_intensity * (1 - grazing_intensity)) * 3.67
            ...
            bgb_change = ((0.19 * feats['crop_area'] * feats['soil_carbon10km']) + (0.08 * feats['tree_area'] * feats['soil_carbon10km'])) * 3.67 * -1 * feats[l + '_area'] / \
                         feats['suitable_area']
        ...
        for l in ['grass_grain']:
            ...
            areas_cultivated = feats['available_area'].values[:, None] * \
                               feats[['ADM0_A3']].merge(foddercrop_area, how="left").drop('ADM0_A3', axis=1).values

            # Keep 9 main fodder crops
            fodder_potential_yields = potential_yields[['climate_bin']+[c +'_potential' for c in foddercrop_list]]
            ...
            potential_prod = areas_cultivated * feats[['climate_bin']].merge(fodder_potential_yields, how="left").drop(
            ...
            if feed_option == 'v1':
                ...
                total_prod = potential_prod * feats[['ADM0_A3']].merge(fodder_yield_fraction, how="left").drop('ADM0_A3',
                                                                                                         axis=1).values
            elif feed_option == 'v2':
                # For feed option v2, keep other uses constant

                # Actual production (t) = Potential production (t) x fraction yield gap x fraction of total grain production going to beef cattle
                total_prod = potential_prod * feats[['ADM0_A3']].merge(fodder_yield_fraction, how="left").drop(
                    'ADM0_A3', axis=1).values - feats[['diff_' + i for i in foddercrop_list]].values

            total_prod = np.where(total_prod < 0, 0, total_prod)

            # Cows eat 80% grain and 20% grass
            grain_perc = 0.5

            grain_prod = np.nansum(total_prod, axis = 1) * grain_perc
            grass_prod = np.nansum(total_prod, axis = 1) * (1-grain_perc)

            grain_area = np.nansum(total_prod * grain_perc / (
                        feats[['climate_bin']].merge(fodder_potential_yields, how="left").drop('climate_bin', axis=1).values *
                        feats[
                            ['ADM0_A3']].merge(fodder_yield_fraction, how="left").drop('ADM0_A3', axis=1).values), axis=1)

            # Get grass area from grass bm consumed and grass yield
            grass_area = grass_prod / feats['grass_0500_N000'].values
            # Cap grass area to from grass bm consumed and grass yield
            grass_area = np.where(grass_area > (feats['available_area'].values - grain_area), (feats['available_area'].values - grain_area), grass_area)
            grass_prod = grass_area * feats['grass_0500_N000'].values

            feats['grain_grassBM'] = grass_prod
            feats['grain_grainBM'] = grain_prod
            ...
            biomass_dom = total_prod * grain_perc * (
                    1 - feats[['ADM0_A3']].merge(percent_exported[['ADM0_A3'] + foddercrop_list], how="left").drop('ADM0_A3', axis=1).values)

            # Biomass consumed for domestic production (t) = actual production (t) x fraction exported feed
            biomass_exported = total_prod * grain_perc * feats[['ADM0_A3']].merge(percent_exported[['ADM0_A3'] + foddercrop_list],
            ...
            meat = (np.nansum(biomass_dom * feed_energy[foddercrop_list].iloc[0].values[None, :], axis=1) + (grass_prod * feats.merge(grass_energy, how='left',
                                 left_on=['region', 'glps'], right_on=['region', 'glps'])['ME'].values)) * \
            ...
            local_methane = (np.nansum(biomass_dom, axis=1) + grass_prod) * \
                            feats[['group', 'glps']].merge(methane_table, how='left', left_on=['group', 'glps'],
                                                           right_on=['region', 'climate'])['me_to_meth'].values

            # Calculate N2O from manure from energy consumed with coefficients (ton CO2eq) = biomass consumed (ton) * conversion factor (ton CO2eq/tom DM)
            local_manure = (np.nansum(biomass_dom, axis=1) + grass_prod) * feats[['group']].merge(grouped_ME, how='left')[
                'nitrous'].values
            ...
            feats[l + '_n2o'] = np.nansum(grain_area[:, None] * feats[['ADM0_A3']].merge(foddercrop_area, how="left").drop('ADM0_A3', axis=1).values * feats[['ADM0_A3']].merge(crop_emissions[['ADM0_A3'] + foddercrop_list], how="left").drop('ADM0_A3', axis=1).values,
                      axis = 1)
            ...
            for f in foddercrop_list:  # Loop though feeds
                ...
                if feed_option == "v2":
                    ...
                    qty_exported = ((grain_area *  feats[['ADM0_A3']].merge(crop_area[['ADM0_A3', f + '_area']], how="left")[f + '_area'].values * \
                                     feats[['climate_bin']].merge(potential_yields[['climate_bin', f + '_potential']],
                                                                  how="left")[f + '_potential'].values * \
                                     feats[['ADM0_A3']].merge(yield_fraction, how="left")[f].values) - feats['diff_' + f].values) * \
                ...
                meat_abroad += np.nansum(qty_exported[:, None] * trade_partners * feed_energy[f].iloc[0] * partner_me['meat'].values[None, :],
                                                      axis=1) * dressing

                ### Methane emitted abroad (t CO2 eq) = Exported feed (t) * partner fraction (%) * methane emissions per biomass consumed (t/t)
                methane_abroad += np.nansum(qty_exported[:, None] * trade_partners *  partner_me["methane"].values[None, :], axis=1)

                ### N2O from manure emitted abroad (t CO2 eq) = Exported feed (t) * partner fraction (%) * N2O emissions per biomass consumed (t/t)
                manure_abroad += np.nansum(qty_exported[:, None] * trade_partners * partner_me["manure"].values[None, :], axis=1)
            ...
            local_cost = np.nansum(
                biomass_dom * feats[['ADM0_A3']].merge(feedprices[['ADM0_A3'] + foddercrop_list], how="left").drop("ADM0_A3", axis=1).values, axis=1)
            ...
            bgb_change_grain = (((-0.59 * feats['pasture_area'] * feats['soil_carbon10km']) + (
                        -0.42 * feats['tree_area'] * feats['soil_carbon10km'])) * 3.67 * -1) * grain_area / \
                         feats['suitable_area']
            bgb_change_grass = ((0.19 * feats['crop_area'] * feats['soil_carbon10km']) + (0.08 * feats['tree_area'] * feats['soil_carbon10km'])) * 3.67 * -1 * grass_area / \
                         feats['suitable_area']
        ...
        for l in ['grass_stover']:
            feats['stover_grassBM'] = feats['available_area'].values * feats['grass_0500_N000'].values

            stover_fraction = feats[['region']].merge(feed_compo[['region', 'stover']], how = 'left')['stover'].values

            stover_max = feats['stover_grassBM'] * (stover_fraction / (1-stover_fraction))
            ...
            feats['stover_stoverBM'] = np.nansum(stover_production, axis=1)

            # Subset meat table for mixed systems
            meat_table = energy_conversion.loc[energy_conversion.feed == 'mixed'][['region', 'climate', beef_yield]]

            # Meat production (t) = Stover energy ('000 MJ) * energy converion (kg/MJ) * dressing percentage
            meat_stover = (stover_energy + feats['stover_grassBM'].values * feats.merge(grass_energy, how='left',
                                 left_on=['region', 'glps'], right_on=['region', 'glps'])['ME'].values) * \
                          feats[['group', 'glps']].merge(meat_table, how='left', left_on=['group', 'glps'],
                                                         right_on=['region', 'climate'])[beef_yield].values * dressing
            ...
            feats[l + '_meth'] = (np.nansum(stover_production, axis=1) + feats['stover_grassBM'].values) * \
                                 feats[['group', 'glps']].merge(methane_table, how='left', left_on=['group', 'glps'],
                                                                right_on=['region', 'climate'])['me_to_meth'].values

            # Manure N20 (t CO2 eq) = Biomass consumed (t) * N2O emissions (t CO2 eq/t)
            feats[l + '_manure'] = (np.nansum(stover_production, axis=1) + feats['stover_grassBM'].values) * \
            ...
            # Estimate carbon content as 47.5% of remaining grass biomass. Then convert to CO2 eq (*3.67)
            grazing_intensity = 0.5
            feats[l + '_cstock'] = 0.475 * (feats['stover_grassBM'].values / grazing_intensity * (1 - grazing_intensity)) * 3.67
            ...
            bgb_change = ((0.19 * feats['crop_area'] * feats['soil_carbon10km']) + (
                        0.08 * feats['tree_area'] * feats['soil_carbon10km'])) * 3.67 * -1 * feats[l + '_area'] / \
                         feats['suitable_area']
            feats[l + '_bgb_change'] = eac(bgb_change, rate=0)
            feats[l + '_bgb_change'] = feats[l + '_bgb_change'].fillna(0)

        for l in ['grain_stover']:
            fodder_potential_yields = potential_yields[['climate_bin'] + [c + '_potential' for c in foddercrop_list]]
            fodder_yield_fraction = yield_fraction[['ADM0_A3'] + foddercrop_list]
            ...
            grain_production = np.where(grain_production < 0, 0, total_prod)

            stover_production = (feats[['diff_' + i for i in foddercrop_list]].values + grain_production) * crop_residues[foddercrop_list].iloc[0].values[None,
                                                                                 :] * stover_removal
            ...
            # # Biomass consumed for domestic production (t) = actual production (t) x fraction exported feed
            # biomass_exported = total_prod * grain_perc* feats[['ADM0_A3']].merge(percent_exported, how="left").drop('ADM0_A3',
            #                                                                                                   axis=1).values
            ...
            # Meat production (t) = sum across feeds (Domestic biomass (t) x ME in feed (MJ/kd DM)) x ME to beef conversion ratio * dressing (%)
            meat = (np.nansum(grain_production * feed_energy[foddercrop_list].iloc[0].values[None, :], axis=1) + (
                np.nansum(stover_production * residue_energy[foddercrop_list].iloc[0].values[None, :], axis=1))) * \
                   feats[['group', 'glps']].merge(meat_table, how='left', left_on=['group', 'glps'],
                                                  right_on=['region', 'climate'])[beef_yield].values * dressing

            meat = np.where(np.nansum(stover_production, axis=1), 0, meat)
            ...
            # Calculate methane produced from local beef production (ton) = biomass consumed (ton) x biomass-methane conversion (ton/ton)
            local_methane = (np.nansum(grain_production, axis=1) + np.nansum(stover_production, axis=1)) * \
                            feats[['group', 'glps']].merge(methane_table, how='left', left_on=['group', 'glps'],
                                                           right_on=['region', 'climate'])['me_to_meth'].values

            # Calculate N2O from manure from energy consumed with coefficients (ton CO2eq) = biomass consumed (ton) * conversion factor (ton CO2eq/tom DM)
            local_manure = (np.nansum(grain_production, axis=1) + np.nansum(stover_production, axis=1))  * \
                           feats[['group']].merge(grouped_ME, how='left')[
                               'nitrous'].values
            ...
            for f in foddercrop_list:  # Loop though feeds
                ...
                if feed_option == "v1":
                    ...
                    qty_exported = ((feats[l + '_area'].values * feats[['ADM0_A3']].merge(
                        foddercrop_area[['ADM0_A3', f + '_area']], how="left")[f + '_area'].values * \
                                     feats[['climate_bin']].merge(fodder_potential_yields[['climate_bin', f + '_potential']],
                                                                  how="left")[f + '_potential'].values * \
                ...
                if feed_option == "v2":
                    ...
                    qty_exported = ((feats[l + '_area'].values * \
                                     ...
                                     feats[['climate_bin']].merge(fodder_potential_yields[['climate_bin', f + '_potential']],
                                                                  how="left")[f + '_potential'].values * \
                                     feats[['ADM0_A3']].merge(fodder_yield_fraction, how="left")[f].values) - feats[
                                        'diff_' + f].values) * \
                                   feats[['ADM0_A3']].merge(percent_exported[['ADM0_A3', f]],
                                                            how="left")[f].values
            ...
            feats[l + '_meat'] = local_meat

            feats[l + '_meth'] = local_methane
            feats[l + '_manure'] = local_manure
            feats[l + '_cost'] = local_cost
            feats[l + '_trans_cost'] = beef_trans_cost
            feats[l + '_trans_emiss'] = beef_trans_emiss
            ...
            bgb_change_grain = (((-0.59 * feats['pasture_area'] * feats['soil_carbon10km']) + (
                    -0.42 * feats['tree_area'] * feats['soil_carbon10km'])) * 3.67 * -1) * (feats[l + '_area'] / \
                               feats['suitable_area'])
            # bgb_change_grass = ((0.19 * feats['crop_area'] * feats['soil_carbon10km']) + (
            #             0.08 * feats['tree_area'] * feats['soil_carbon10km'])) * 3.67 * -1 * grass_area / \
            #                    feats['suitable_area']
            ...
            for feed in ["grass", "grain", "stover"]:
                ...
                if feed == 'grass':
                    ...
                    for f in crop_list:
                        ### Meat produced abroad
                        # Quantity of feed f exported
                        qty_exported = ((feats[l + '_' + feed + '_' + "area"].values * \
                                         feats[['ADM0_A3']].merge(crop_area[['ADM0_A3', f + '_area']], how="left")[f + '_area'].values * feats[['climate_bin']].merge(
                        ...
                        # Select trade partners for each cell
                        trade_partners = feats[['ADM0_A3']].merge(feedpartners.loc[feedpartners.crop == f],
                                                                  how='left').drop(['ADM0_A3', 'crop'], axis=1).values

                        meat_abroad += np.nansum(qty_exported[:, None] * trade_partners * feed_energy[f].iloc[0] * partner_me['meat'].values[None, :], axis=1)

                        ### Methane emitted abroad
                        methane_abroad += np.nansum(qty_exported[:, None] * trade_partners * \
                                                                    partner_me["methane"].values[None, :], axis=1)

                        ### N20 emitted from manure abroad
                        manure_abroad += np.nansum(qty_exported[:, None] * trade_partners * \
                                                                  partner_me["manure"].values[None, :], axis=1)
                        ...
                        trancost_partner_ls += np.nansum(ntrips_local_transp * exp_access['access'].values[None, :] *
                                                         fuel_partner['Diesel'].values[None,:] * fuel_efficiency / 1000.,axis=1)

                        emissions_partner_ls += np.nansum(ntrips_local_transp * exp_access['access'].values[None,:] * fuel_efficiency * truck_emission_factor / 1000., axis=1)

                        ### Local transport emissions in importing country

                    local_cost = np.nansum(
                        biomass_dom * feats[['ADM0_A3']].merge(feedprices[['ADM0_A3'] + foddercrop_list], how="left").drop("ADM0_A3", axis=1).values,
                        axis=1)
                    ...
                    feats[l + '_' + feed + '_beefarea'] = np.nansum(actual_prod / (feats[['climate_bin']].merge(
                        potential_yields, how="left").drop('climate_bin',axis=1).values * feats[['ADM0_A3']].merge(yield_fraction, how="left").drop(
                                           'ADM0_A3', axis=1).values), axis=1)
                    ...
                    stover_energy = np.nansum(stover_production * residue_energy.iloc[0].values[None, :], axis=1) * (feats[l + '_' + feed + '_' + "area"] / feats["suitable_area"])
                    ...
                    meat = np.sum(np.where(feats[months] < -1,
                                                 (meat[:, None] - (
                                                         meat[:, None] * (
                                                         -0.0182 * feats[months] - 0.0182))) / 12.,
                                                 meat[:, None] / 12.), axis=1) * dressing
                    ...
                    methane = np.nansum(stover_production, axis=1) * \
                                    feats[['group', 'glps']].merge(me_table, how='left', left_on=['group', 'glps'],
                                                                   right_on=['region', 'climate'])['me_to_meth'].values

                    manure = np.nansum(stover_production, axis=1) * \
                                   feats[['group']].merge(grouped_ME, how='left')[
                                       'nitrous'].values
                    feats[l + '_' + feed + '_meat'] = meat
                    feats[l + '_' + feed + '_meth'] = methane
                    feats[l + '_' + feed + '_manure'] = manure
    ...
    for l in landuses:
        ...
        feats[l + '_tot_cost'] = feats[l + '_est_cost'] + feats[l + '_cost'] + feats[l + '_trans_cost'] + feats[l + '_opp_cost'] + feats[l + '_compensation'] + feats['aff_cost']
...
def trade(feats, optimisation_method, lam, feed_option):
    ...
    for l in landuses:
        ...
        feats[l + '_tot_cost'] = feats['est_cost'] + (
                feats[l + '_cost'] + feats[l + '_trans_cost'] + feats[l + '_opp_cost'] + feats[l + '_exp_costs']  + feats[l + '_compensation'])
...
def main(export_folder ='.', optimisation_method= 'weighted_sum', lam = 0.5, demand_scenario = 'Demand',
    ...
    # grid = pd.read_sql_query("SELECT * FROM grid WHERE ADM0_A3 ='FRA'", conn)
    # conn.close()
    ...
    grid = scoring(grid, optimisation_method, crop_yield, lam, beef_yield, aff_scenario, logger, feed_option)
    ...
    grid['grass_BM'] = np.select([grid.bestlu == 9, grid.bestlu == 10, grid.bestlu == 11],
                                 [grid.grain_grassBM, grid.stover_grassBM, 0],
                                 default=grid.grass_grassBM)
    grid['grain_BM'] = np.select([grid.bestlu == 9, grid.bestlu == 11],
                                 [grid.grain_grainBM, grid.stover_grain_grainBM],
                                 default = 0)
    grid['stover_BM'] = np.select([grid.bestlu == 10, grid.bestlu == 11],
                                  [grid.stover_stoverBM, grid.stover_grain_stoverBM],
                                  default=0)

    # grid['stover_BM'] = np.where(grid.bestlu == 11, grid.stover_stoverBM, 0)
    ...
    grid[cols + ['grass_BM', 'grain_BM','stover_BM']].to_file(export_folder + '/' + constraint + "_" + str(crop_yield) + '_' + beef_yield + '_' + str(lam) + '_' + dem + "_" + feed_option + "_" + aff_scenario + ".gpkg", driver="GPKG")
        ...
        for j in ['me_to_meat', 'max_yield']:
            for k in [0, 1]:
                for d in ['SSP1-NoCC2010', 'SSP1-NoCC2050']:
                    for a in ['no_aff', 'nat_aff', 'managed_aff']:
