...
def scoring(feats, optimisation_method, crop_yield, lam, beef_yield, aff_scenario, logger, feed_option, landuses):
    ...
    #
    # available_area = np.nansum(np.where(attainable_prod < 0, 0, attainable_prod) / (
    #         feats[['climate_bin']].merge(potential_yields, how="left").drop('climate_bin', axis=1).values * feats[
    ...
    # Adjust yields with yield fractions:
    # for crop in foddercrop_list:
    #     feats[crop] = feats[crop] * feats[['ADM0_A3']].merge(yield_fraction[['ADM0_A3',crop]], how="left")[crop].values

    ###### Updated calculation of available area based on sum (current crop area - current beef fodder area) ######
    # feats['available_area'] = np.where(feats['suitable_area'].values - feats['net_fodder_area'].values < 0, 0,
    #          feats['suitable_area'].values - feats['net_fodder_area'].values)
    #
    # feats = feats.loc[feats.available_area > 0]

    ### Available area based on current cropping area
    feats['available_area'] = np.where(feats['suitable_area'].values < feats['net_fodder_area'].values, 0,
             feats['suitable_area'].values - feats['net_fodder_area'].values)

    ### Available area based on current feed frop and grazing area
    # feats['available_area'] = feats['current_grazing'].fillna(0).values + feats['current_cropping'].fillna(0).values
    logger.info(feats[['available_area', 'current_grazing', 'current_cropping']].dtypes)

    logger.info(feats[['current_grazing', 'current_cropping']])

    logger.info('Available_area: {}'.format(feats[['area', 'available_area']]))
    logger.info('Sum Available_area: {}'.format(feats['available_area'].sum()))

    logger.info('-------')

    # feats['available_area'] = np.where(feats.area < feats.available_area, feats.area, feats.available_area)
    # feats['available_area'] = np.where(feats['available_area'].values > feats.area, feats.area, feats['available_area'].values)

    logger.info('Available_area: {}'.format(feats['available_area']))
    logger.info('Sum Available_area: {}'.format(feats['available_area'].sum()))
    ...
    opp_soc = feats['current_grazing'].fillna(0).values * feats['bgb_spawn'].values * soc_change[
        'grassland'].values * -1 + feats['current_cropping'].fillna(0).values * feats[
        'bgb_spawn'].values * soc_change['cropland'].values * -1

    # Make sure that opportunity cost of soil carbon is only where there currently is beef production
    opp_soc = np.where(feats.bvmeat.values > 0, opp_soc, 0) * 3.67
    ...
    #     (feats['current_grazing'].fillna(0).values + feats['current_cropping'].fillna(0).values) > feats[
    #         'available_area'].values,
    #     (feats['current_grazing'].fillna(0).values + feats[
    #         'current_cropping'].fillna(0).values) - feats['available_area'].values,
    #     0)
    regrowth_area = feats['current_grazing'].fillna(0).values + feats['current_cropping'].fillna(0).values

    logger.info('regrowth_area 1: {}'.format(regrowth_area))

    # regrowth_area = np.where(regrowth_area > feats.area, feats.area, regrowth_area)

    # logger.info('regrowth_area 2: {}'.format(regrowth_area))
    ...
    if feed_option in ['v1', 'v2']:
        ...
        for l in grass_cols:
            ...
            feats[l + '_cstock'] = (feats['agb_spawn'] * 3.67 * extra_area) - \
                                   (0.475 * ((feats[l].values * (feats[l + '_area'].values - feats['current_grazing'])) / grazing_intensity * (1 - grazing_intensity)) * 3.67)
        ...
        for l in ['grass_grain']:
            ...
            # feats[l + '_cstock'] = 0
            extra_area = np.where(feats[l + '_area'].values <= regrowth_area , 0, feats[l + '_area'].values - regrowth_area)
            cstock_grain = (feats['agb_spawn'] * 3.67 * extra_area)
        ...
        for l in ['stover_grass']:
            ...
            #                                                                      :] * stover_removal
            potential_stover = feats['stover_bm'].values
        ...
        for l in ['stover_grain']:
            ...
            # Meat production (t) = sum across feeds (Domestic biomass (t) x ME in feed (MJ/kd DM)) x ME to beef conversion ratio * dressing (%)
            stover_meat = feats['stover_energy'].values * stov_adjustment * \
                   feats[['group', 'glps']].merge(subset_table, how='left', left_on=['group', 'glps'],
                                                  right_on=['group', 'glps'])[beef_yield].values * dressing
    ...
    #                 feats[l + '_' + feed + '_n2o'] = (n_applied * grass_n2o_factor["grass"]) * 298
    ...
    #                     l + '_' + feed + '_area']
    #
    #                 bgb_change = ((0.19 * feats['crop_area'] * feats['bgb_spawn']) + (
    #                         0.08 * feats['tree_area'] * feats['bgb_spawn'])) * 3.67 * -1
    ...
    #                                 potential_yields[['climate_bin', f]], how="left")[f].values * \
    ...
    #                     l + '_' + feed + '_beefarea']
    #
    #                 bgb_change = (((-0.59 * feats['pasture_area'] * feats['bgb_spawn']) + (
    #                         -0.42 * feats['tree_area'] * feats['bgb_spawn'])) * 3.67 * -1) * feats[l + '_' + feed + '_beefarea'] / feats['suitable_area']
    #                 feats[l + '_' + feed + '_bgb_change'] = eac(bgb_change, rate=0)
    #
    #             elif feed == 'stover':
    #
...
def trade(feats, optimisation_method, lam, feed_option, landuses):
    ...
    for l in landuses:
        ...
        feats[l + '_tot_cost'] = feats[l + '_est_cost'] + (
...
def main(export_folder ='.', optimisation_method= 'weighted_sum', lam = 0.5, demand_scenario = 'Demand',
    ...
    if trade_scenario == 'trade':
        ...
        while total_production < demand and grid.loc[(grid.changed == 0)].shape[0] > 0 and new_production != 0:
            ...
            if constraint in ['global', 'subsistence']:
                ...
                test = grid.loc[(grid.changed == 1)][['ADM0_A3', 'dom_production', 'production']].groupby(
                    by='ADM0_A3', as_index=False).agg({'dom_production': 'mean',
                                                       'production': 'sum'})
                # logger.info(grid[['ADM0_A3', 'dom_production', 'production', 'changed']])
                # logger.info(test)
            ...
            # logger.info(grid[['production', 'bestlu', 'best_score', 'changed', 'available_area','cumdomprod','dom_demand','dom_production']])
            # logger.info("Total production: {}, demand: {}".format(total_production, demand))
            # logger.info("Number of unconverted cells: {}".format(grid.loc[(grid.changed == 0)].shape[0]))
    ...
    # forest_type = np.select(
    #     [np.isin(grid.loc[grid.changed == 0, 'ecoregions'].values, [1, 2, 3, 7, 9]), np.isin(grid.loc[grid.changed == 0, 'ecoregions'].values, [4, 5, 8, 12]),
    #      np.isin(grid.loc[grid.changed == 0, 'ecoregions'].values, [6, 10, 11])],
    #     [1, 2, 3], default=0)
    #
    # regrowth_area = np.where(
    #     grid.loc[grid.changed == 0, 'current_grazing'].fillna(0).values + grid.loc[grid.changed == 0, 'current_cropping'].fillna(0).values > grid.loc[grid.changed == 0, 'area'],
    #                          grid.loc[grid.changed == 0, 'area'],
    #     grid.loc[grid.changed == 0, 'current_grazing'].fillna(0).values + grid.loc[
    #         grid.changed == 0, 'current_cropping'].fillna(0).values)
    #
    # max_cstock = regrowth_area * grid.loc[grid.changed == 0, 'potential_carbon'].values * -1
    #
    # if aff_scenario == 'noaff':
    #     opp_aff = 0
    #     aff_cost = 0
    ...
    #
    #     opp_aff = max_cstock * (1 - np.exp(-k * t)) ** p
    #     opp_aff = eac(opp_aff, rate=0) * 3.67
    #
    #     aff_cost = 0
    #
    # elif aff_scenario == 'manaff':
    ...
    #     opp_aff = max_cstock * (1 - np.exp(-k * t)) ** p
    #     opp_aff = eac(opp_aff, rate=0) * 3.67
    #     initial_aff_cost = grid.loc[grid.changed == 0][['ADM0_A3']].merge(aff_costs, how='left')['initial'].values / 1000. * regrowth_area
    #     annual_aff_cost = grid.loc[grid.changed == 0][['ADM0_A3']].merge(aff_costs, how='left')['annual'].values / 1000. + regrowth_area
    #     aff_cost = eac(initial_aff_cost) + annual_aff_cost
    #
    # else:
    #     logger.info("Afforestation scenario {} not in choices".format(aff_scenario))

    # total_opp_aff = np.nansum(opp_aff)  + grid.loc[grid.changed == 1, 'opp_aff'].sum()
    # total_aff_costs = np.nansum(aff_cost) + grid.loc[grid.changed == 1, 'aff_cost'].sum()
    ...
    # prod_loss.to_csv(export_folder + '/prod_loss_' + constraint + "_" + str(crop_yield) + "_" + beef_yield + "_" + str(lam) + '_' + dem + "_" + aff_scenario + ".csv", index=False)
    ...
    sub = grid.iloc[1:20].drop('geometry', axis = 1)
    sub.to_csv(export_folder + '/sub_' + constraint + "_" + str(crop_yield) + "_" + beef_yield + "_" + str(lam) + '_' + dem + "_" + feed_option + "_" + aff_scenario + ".csv", index=False)
    ...
    #
    # # Write scenarios
    # scenario_columns = {"optimisation_method": [optimisation_method], "constraint": [constraint], "weight": [str(lam)],
    ...
    totaldf = pd.DataFrame({"suitable_area": grid.suitable_area.sum(),
                            ...
                            "total_emissions": grid.total_emissions.sum(),
                            "total_costs": grid.total_costs.sum(),
    ...
    # print('---')
    # print(grid[['grass_BM', 'grain_BM', 'stover_BM', 'biomass']])
    # print(grid['grain_BM']/grid['biomass'])
    # print('---')

    # Export aggregate results in csv file
    totaldf.to_csv(export_folder + '/total_' + constraint + "_" + str(crop_yield) + "_" + beef_yield + "_" + str(lam) + '_' + dem + "_" + feed_option + "_" + aff_scenario + ".csv", index=False)
    ...
    newcols = cols + ['grass_BM', 'grain_BM', 'stover_BM','grass_meat', 'grain_meat', 'stover_meat']
    # for c in newcols:
    #     logger.info(c)
    # logger.info(newcols)
    grid = grid[newcols]
    grid.to_file(export_folder + '/' + constraint + "_" + str(crop_yield) + '_' + beef_yield + '_' + str(lam) + '_' + dem + "_" + feed_option + "_" + aff_scenario + ".gpkg", driver="GPKG")

    # logger.info(grid[['bestlu', 'total_emission', 'n2o_emissions']])
    # logger.info(grid[newcols].head())
    # grid.drop('geometry', axis = 1).to_csv(export_folder + '/grid_' + constraint + "_" + str(crop_yield) + '_' + beef_yield + '_' + str(lam) + '_' + dem + "_" + feed_option + "_" + aff_scenario + ".csv")
    ...
    country_data.to_csv(export_folder + '/country_' + constraint + "_" + str(crop_yield) + "_" + beef_yield + "_" + str(lam) + '_' + dem + "_" + aff_scenario + ".csv", index=False)
    ...
    # grid.to_file('./test.gpkg', driver="GPKG")
    # grid.to_file('./test.geojson', driver="GeoJSON")

    # grid.to_file(export_folder + '/' + constraint + "_" + str(crop_yield) + '_' + beef_yield + '_' + str(lam) + '_' + dem + "_" + feed_option + "_" + aff_scenario + ".gpkg", driver="GPKG")
    # grid.to_file(export_folder + '/' + constraint + "_" + str(crop_yield) + '_' + beef_yield + '_' + str(lam) + '_' + dem + "_" + feed_option + "_" + aff_scenario + ".geojson", driver="GeoJSON")
