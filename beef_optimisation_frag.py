...
def scoring(feats, optimisation_method, crop_yield, lam, beef_yield, aff_scenario, logger, feed_option, landuses):
    ...
    if feed_option in ['v1', 'v2']:
        ...
        for l in grass_cols:
            ...
            extra_area = np.where(feats[l + '_area'] - regrowth_area < 0, 0, feats[l + '_area'] - regrowth_area)
            feats[l + '_cstock'] = (feats['agb_spawn'] * 3.67 * extra_area) - (0.475 * (feats[l + '_BM'] / grazing_intensity * (1 - grazing_intensity)) * 3.67)
        ...
        for l in ['grass_grain']:
            ...
            # feats[l + '_cstock'] = 0
            extra_area = np.where(grain_area - regrowth_area < 0, 0, feats[l + '_area'] - regrowth_area)
...
def main(export_folder ='.', optimisation_method= 'weighted_sum', lam = 0.5, demand_scenario = 'Demand',
    ...
    # grid.drop(del_cols, axis = 1).to_file(export_folder + '/' + constraint + "_" + str(crop_yield) + '_' + beef_yield + '_' + str(lam) + '_' + dem + "_" + feed_option + "_" + aff_scenario + ".gpkg", driver="GPKG")
