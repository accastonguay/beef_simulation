...
croplists = {'all' : crop_list,
             'sub': foddercrop_list}

cropareas = {'all' : crop_area,
             'sub': foddercrop_area}

def eac(cost, rate = 0.05, lifespan = 30.):
    """
...
def scoring(feats, optimisation_method, crop_yield, lam, beef_yield, aff_scenario, logger, feed_option, crops):
    """
    ...
    beef_yield (str)-> Scenario of beef yield ('curr' = current, 'max' = no yield gap)
    ...
    logger.info(yield_fraction.head())
    ...
    # Calculate available area on a cell given grain production for other uses
    attainable_prod = (feats['suitable_area'].values[:, None] * \
                       feats[['ADM0_A3']].merge(crop_area, how="left").drop('ADM0_A3', axis=1).values * \
                       feats[['climate_bin']].merge(potential_yields, how="left").drop(
        'climate_bin', axis=1).values * feats[['ADM0_A3']].merge(yield_fraction, how="left").drop(
        'ADM0_A3', axis=1).values) - feats[['diff_' + i for i in crop_list]].values

    available_area = np.nansum(np.where(attainable_prod < 0, 0, attainable_prod) / (feats[['climate_bin']].merge(potential_yields, how="left").drop(
        'climate_bin', axis=1).values * feats[['ADM0_A3']].merge(yield_fraction, how="left").drop(
        'ADM0_A3', axis=1).values), axis = 1)

    available_area = np.where(feats.suitable_area == 0, 0, available_area)
    available_area = np.where(available_area > feats.suitable_area, feats.suitable_area, available_area)

    feats['available_area'] = available_area
    feats = feats.loc[feats.available_area > 0]

    if feed_option in ['v1', 'v2']:
        landuses = grass_cols + ['grain', 'stover']  # landuses to include in the simulation
        ...
        for l in grass_cols:
            ...
            bgb_change = ((0.19 * feats['crop_area'] * feats['soil_carbon10km']) + (0.08 * feats['tree_area'] *feats[
                'soil_carbon10km'])) * 3.67 * -1 * feats[l + '_area'] / feats['suitable_area']
        ...
        for l in ['grain']:
            ...
            # Production for all 16 crops
            grain_prod = feats[l + '_area'].values[:, None] * feats[['ADM0_A3']].merge(
                cropareas[crops], how="left").drop('ADM0_A3', axis=1).values *  feats[['climate_bin']].merge(
                potential_yields[['climate_bin'] + [c + '_potential' for c in croplists[crops]]], how="left").drop(
                'climate_bin', axis=1).values * feats[['ADM0_A3']].merge(
                yield_fraction[['ADM0_A3'] + croplists[crops]], how="left").drop('ADM0_A3', axis=1).values

            feats[l + '_BM'] = np.nansum(grain_prod, axis=1)

            biomass_dom = grain_prod * (1 - feats[['ADM0_A3']].merge(percent_exported[['ADM0_A3'] + croplists[crops]],
            ...
            biomass_exported = grain_prod * feats[['ADM0_A3']].merge(percent_exported[['ADM0_A3'] + croplists[crops]],
                                                                     how="left").drop('ADM0_A3', axis=1).values

            # Subset ME in conversion, methane and manure per region and climate
            ...
            meat = np.nansum(biomass_dom * feed_energy[croplists[crops]].iloc[0].values[None, :], axis=1) * \
                   feats[['group', 'glps']].merge(subset_table, how='left', left_on=['group', 'glps'],
                                                  right_on=['group', 'glps'])[beef_yield].values * dressing

            # Update meat production after climate penalty
            local_meat = np.sum(np.where(feats[months] < -1, (meat[:, None] - (meat[:, None] * (
                    -0.0182 * feats[months] - 0.0182))) / 12., meat[:, None] / 12.), axis=1)

            # Calculate methane produced from local beef production (ton) = biomass consumed (ton) x biomass-methane conversion (ton/ton)
            local_methane = np.nansum(biomass_dom, axis=1) * feats[['group', 'glps']].merge(
                subset_table, how='left', left_on=['group', 'glps'], right_on=['group', 'glps'])['curr_methane'].values

            # Calculate N2O from manure from energy consumed with coefficients (ton CO2eq) = biomass consumed (ton) * conversion factor (ton CO2eq/tom DM)
            local_manure = np.nansum(biomass_dom, axis=1) * feats[['group', 'glps']].merge(
                subset_table, how='left', left_on=['group', 'glps'], right_on=['group', 'glps'])['curr_manure'].values

            # Calculate nitrous N2O (ton) = Actual production (ton) x fertiliser requirement (kg) x crop_emission factors (% per thousand)
            feats[l + '_n2o'] = np.nansum(feats[l + '_area'].values[:, None] * feats[['ADM0_A3']].merge(
                cropareas[crops], how="left").drop('ADM0_A3', axis=1).values * feats[['ADM0_A3']].merge(
                    crop_emissions[['ADM0_A3'] + croplists[crops]], how="left").drop('ADM0_A3', axis=1).values, axis=1)
            ...
            for f in croplists[crops]:  # Loop though feeds
                ...
                if feed_option == "v1":
                    ...
                    qty_exported = ((feats[l + '_area'].values * feats[['ADM0_A3']].merge(
                        cropareas[crops][['ADM0_A3', f + '_area']], how="left")[f + '_area'].values * \
                                   ...
                                   feats[['ADM0_A3']].merge([['ADM0_A3', f]], how="left")[f].values

                if feed_option == "v2":
                    # Qty exported (t) = (Suitable area (ha) * crop area fraction * crop yield (t/ha) * yield gap (%)) - production for other uses (t) * export fraction
                    qty_exported = ((feats[l + '_area'].values * feats[['ADM0_A3']].merge(
                        cropareas[crops][['ADM0_A3', f + '_area']], how="left")[f + '_area'].values * \
                                     feats[['climate_bin']].merge(
                                         potential_yields[['climate_bin', f + '_potential']], how="left")[f + '_potential'].values * \
                                     feats[['ADM0_A3']].merge(yield_fraction, how="left")[f].values) - feats[
                                        'diff_' + f].values) * \
                ...
                meat_abroad += np.nansum(
                    qty_exported[:, None] * trade_partners * feed_energy[f].iloc[0] * partner_me['meat'].values[
                                                                                      None, :], axis=1) * dressing
                ...
                trancost_partner_ls += np.nansum(ntrips_local_transp * exp_access['access'].values[
                                                                       None, :] * fuel_partner['Diesel'].values[None,
                ...
                emissions_partner_ls += np.nansum(ntrips_local_transp * exp_access['access'].values[
                                                                        None, :] * fuel_efficiency * truck_emission_factor / 1000., axis=1)
            ...
            local_cost = np.nansum(biomass_dom * feats[['ADM0_A3']].merge(feedprices[['ADM0_A3'] + croplists[
                crops]], how="left").drop("ADM0_A3", axis=1).values, axis=1)
            ...
            feed_to_port_emis = ntrips_feed_exp * feats['distance_port'] * fuel_efficiency * truck_emission_factor/1000.
            ...
            beef_trans_cost = ntrips_beef_mkt * feats[['ADM0_A3']].merge(
                fuel_cost[['ADM0_A3', 'Diesel']], how='left', left_on='ADM0_A3', right_on='ADM0_A3')['Diesel'].values * \
                              feats["accessibility"] * fuel_efficiency / 1000.
            ...
            bgb_change_grain = (((-0.59 * feats['pasture_area'] * feats['soil_carbon10km']) + (
        ...
        for l in ['stover']:
            ...
            potential_stover = feats[['diff_' + i for i in croplists[crops]]].values * crop_residues[croplists[crops]].iloc[0].values[None,
                                                                                 :] * stover_removal

            # Stover energy ('000 MJ) = sum across rows of Stover production (t) * stover energy (MJ/kg dm)
            stover_energy = np.nansum(potential_stover * residue_energy[croplists[crops]].iloc[0].values[None, :], axis=1)

            feats[l + '_BM'] = np.nansum(potential_stover, axis=1)
            ...
            # Carbon stock for stover = current carbon stock
            feats[l + '_cstock'] = feats['carbon_stock'] * feats['suitable_area'] * 3.67
    ...
    forest_type = np.select(
        [np.isin(feats.ecoregions, [1, 2, 3, 7, 9]), np.isin(feats.ecoregions, [4, 5, 8, 12]), np.isin(feats.ecoregions, [6, 10, 11])],
        [1, 2, 3], default=0)

    regrowth_area = np.where(
        (feats['current_grazing'].fillna(0).values + feats['current_cropping'].fillna(0).values) > feats['available_area'].values,
        (feats['current_grazing'].fillna(0).values + feats['current_cropping'].fillna(0).values) - feats['available_area'].values,
        0)
    ...
    if aff_scenario == 'noaff':
        ...
        k = np.select([forest_type == 1, forest_type == 2, forest_type == 3],
                      [ 0.02, 0.02,  0.02], default = 0)
        p = np.select([forest_type == 1, forest_type == 2, forest_type == 3],
                      [2, 5, 5], default = 0)
        t = 100.
        ...
        initial_aff_cost = feats[['ADM0_A3']].merge(aff_costs, how = 'left')['initial'].values * regrowth_area
        annual_aff_cost = feats[['ADM0_A3']].merge(aff_costs, how = 'left')['annual'].values + regrowth_area
    ...
    for l in landuses:
        # Annualise establishment cost. A time horizon of 30 years has been adopted, since this is commonly practiced in agricultural investment (Wang et al 2016 Ecol Econ.)
        feats[l + '_est_cost'] = eac(feats['est_cost'] * feats[l + '_area'] / feats['suitable_area'])

        # Calculate opportunity cot in '000 USD
        feats[l + '_opp_cost'] = feats['opp_cost'].astype(float) / 1000. * feats[l + '_area']

        # Compensation for loss of revenues from beef ('000 USD) = max(0, beef_prices ('000 USD) x (current_prod - new_prod))
        ...
        feats[l + '_tot_cost'] = feats[l + '_est_cost'] + feats[l + '_cost'] + feats[l + '_trans_cost'] + feats[l + '_opp_cost'] + feats['aff_cost']
...
def trade(feats, optimisation_method, lam, feed_option):
    ...
    for l in landuses:
        ...
        feats[l + '_tot_cost'] = feats[l + '_est_cost'] + feats[l + '_cost'] + feats[l + '_trans_cost'] + feats[l + '_opp_cost']  + feats['aff_cost'] + feats[l + '_exp_costs']
                # + feats[l + '_compensation'])
...
def main(export_folder ='.', optimisation_method= 'weighted_sum', lam = 0.5, demand_scenario = 'Demand',
         crop_yield = 0, beef_yield ='curr', constraint ='global', aff_scenario = "noaff", feed_option ='v1',
         crops = 'sub', trade_scenario ='trade'):
    """
    ...
    try:
        logging.basicConfig(
            ...
            filename="/home/uqachare/model_file/test_us_" + trade_scenario + ".log",
            level=logging.INFO,
            format=LOG_FORMAT,
            filemode='w')
    except:
    ...
    # grid = grid.loc[grid.ADM0_A3 == 'USA']
    ...
    # logger.info(grid.loc[grid.ADM0_A3 == 'GRL'][['geometry', 'diff_potato']])
    ...
    # grid[['diff_' + i for i in crop_list]] = grid[['diff_' + i for i in crop_list]] * other_uses['dem_increase'].values[:,None]
    del other_uses

    logger.info('New demand for other uses after: {}'.format(grid[['diff_maize']].loc[grid.diff_maize > 0].head()))

    grid = scoring(grid, optimisation_method, crop_yield, lam, beef_yield, aff_scenario, logger, feed_option, crops)
    ...
    elif trade_scenario == 'feed_constraint':
        pd.set_option('display.max_columns', None)


        grid1 = grid[['ADM0_A3', 'best_score', 'bestlu','production', 'biomass', 'changed']].copy()
        grid1['id'] = range(0,grid1.shape[0])
        grid1['o_bs'] = grid1['best_score']
        grid1['o_bl'] = grid1['bestlu']
        grid1['o_prod'] = grid1['production']
        grid1['o_bm'] = grid1['biomass']

        grid1['g_bs'] = np.nanmin(grid[[l + '_score' for l in grass_cols]].values, axis=1)

        array = np.ma.masked_array(grid[[l + '_score' for l in grass_cols]].values, np.isnan(grid[[l + '_score' for l in grass_cols]].values))

        grid1['g_bl'] = np.nanargmin(array, axis=1)

        for i, j in zip(['production', 'biomass'], ['g_prod', 'g_bm']):
            grid1[j] = np.take_along_axis(grid[[l + new_colnames[i] for l in grass_cols]].values,
                                          grid1['g_bl'].values[:, None], axis=1)


        # grid1['grass'] = np.where(grid1.bestlu <= 8, grid1.biomass, 0)
        grid1['grain'] = np.where(grid1.bestlu == 9, grid1.biomass, 0)
        grid1['stover'] = np.where(grid1.bestlu == 10, grid1.biomass, 0)

        # grid1.loc[grid1.changed == 0, 'grass_cum'] = grid1.groupby('ADM0_A3')['grass'].transform(pd.Series.cumsum)
        grain_cum = grid1.groupby('ADM0_A3')['grain'].transform(pd.Series.cumsum)
        stover_cum = grid1.groupby('ADM0_A3')['stover'].transform(pd.Series.cumsum)

        cumBM = grid1.groupby('ADM0_A3')['biomass'].transform(pd.Series.cumsum)

        # grid1['grass_frac'] = grid1['grass_cum'] / grid1['cumBM']
        grid1['grain_frac'] = grain_cum / cumBM
        grid1['stover_frac'] = stover_cum / cumBM

        grid1['cumprod'] = grid1['production'].cumsum()

        old_production = grid1.loc[grid1.changed == 1, 'production'].sum()

        # while total_production < demand and grid.loc[(grid.changed == 0)].shape[0] > 0 and new_production != 0:
        for i in range(grid.shape[0]):

            # logger.info(grid[['ADM0_A3', 'fid', 'biomass', 'cumBM', 'grain_frac', 'stover_frac', 'stop','best_score', 'bestlu', 'production', 'cumprod', 'changed']])
            # logger.info('---')
            logger.info('Start')
            # logger.info(grid1[['ADM0_A3', 'grain_frac', 'stover_frac', 'changed']])
            # If constraints are met, convert cells
            if (grid1.loc[grid1.changed == 0, 'grain_frac'].iloc[0] < 0.8) & (
                    grid1.loc[grid1.changed == 0, 'stover_frac'].iloc[0] < 0.7):

                for i, s, g in zip(range(len(grid1.loc[grid1.changed == 0])), grid1.loc[grid1.changed == 0, 'stover_frac'],
                                   grid1.loc[grid1.changed == 0, 'grain_frac']):
                    # print(i,s,g)
                    # print(grid1.stover_frac.iloc[i], grid1.grain_frac.iloc[i])
                    if grid1.stover_frac.iloc[i] > 0.70 or grid1.grain_frac.iloc[i] > 0.80:
                        bs = grid1.best_score.iloc[i]
                        break

                grid1.loc[(grid1.changed == 0) &
                         ((demand + grid1['production'] - grid1['cumprod']) > 0) &
                         (grid1.best_score < bs), 'changed'] = 1

                try:
                    country = grid1.loc[grid1.best_score == grid1.loc[grid1.changed == 0, 'best_score'].min(), 'ADM0_A3'].iloc[0]
                except:
                    logger.info("grid1 empty")

                # logger.info('Country: {}'.format(country))

            # If constraints are not met
            else:
                # Select country of first cell
                country = grid1.loc[(grid1['changed'] == 0), 'ADM0_A3'].iloc[0]
            logger.info('Converted 1')

            grid1['best_score'] = np.where((grid1.changed == 0) & (grid1.ADM0_A3 == country), grid1.g_bs, grid1.best_score)
            grid1['best_lu'] = np.where((grid1.changed == 0) & (grid1.ADM0_A3 == country), grid1.g_bl, grid1.bestlu)

            logger.info('Set grass score and lu')

            if (grid1.loc[grid1.changed == 1, 'production'].sum() < demand) & (grid1.loc[(grid1.changed == 0)].shape[0] > 0):

                idmin = grid1.loc[grid1.best_score == grid1.loc[(grid1.changed == 0) & (grid1.ADM0_A3 == country), 'best_score'].min(), 'id'].iloc[0]
                grid1['production'] = np.where((grid1.changed == 0) & (grid1.ADM0_A3 == country), grid1.g_prod,
                                               grid1.production)
                grid1['biomass'] = np.where((grid1.changed == 0) & (grid1.ADM0_A3 == country), grid1.g_bm, grid1.biomass)

                grid1.loc[grid1.id == idmin, 'grain'] = 0
                grid1.loc[grid1.id == idmin, 'stover'] = 0
                grid1.loc[grid1.id == idmin, 'changed'] = 1

            logger.info('Converted single')

            grid1['best_score'] = grid1.o_bs
            grid1['bestlu'] = grid1.o_bl
            grid1['production'] = grid1.o_prod
            grid1['biomass'] = grid1.o_bm

            logger.info('Reverted to initial')

            grid1 = pd.concat([grid1.loc[grid1.changed == 1], grid1.loc[grid1.changed == 0].sort_values('best_score')])

            logger.info('Concat grid1')

            # grid1['grain'] = np.where(grid1.bestlu.values == 9, grid1.biomass.values, 0)
            # grid1['stover'] = np.where(grid1.bestlu.values == 10, grid1.biomass.values, 0)
            ...
            total_production = grid1.loc[grid1.changed == 1, 'production'].sum()

            # new_production = round(total_production, 3) - round(old_production, 3)

            logger.info('total_production: {}'.format(total_production))
            logger.info('Converted cells: {}'.format(grid1.loc[grid1.changed == 1].shape[0]))
    ...
    logger.info("Total production: {}".format(grid.production.sum()))
    ...
    total_aff_costs = grid['aff_cost'].sum()
    ...
    newdf.to_csv(export_folder + '/' + constraint + "_" + str(crop_yield) + "_" + beef_yield + "_" + str(lam) + '_' + dem + "_" + feed_option + "_" + aff_scenario + ".csv", index=False)
    logger.info("Exporting CSV finished")

    # Export database results in gpkg file
    grid[cols].to_file(export_folder + '/' + constraint + "_" + str(crop_yield) + '_' + beef_yield + '_' + str(lam) + '_' + dem + "_" + feed_option + "_" + aff_scenario + ".gpkg", driver="GPKG")
    # grid.to_file(export_folder + '/' + constraint + "_" + str(crop_yield) + '_' + beef_yield + '_' + str(lam) + '_' + dem + "_" + feed_option + ".gpkg", driver="GPKG")

    logger.info("Exporting GPKG finished")

    # Export raster
    export_raster(grid, 0.0833, ['production'], export_folder, constraint, crop_yield, beef_yield, lam, dem, feed_option, aff_scenario)
    logger.info("Exporting raster finished")

def parallelise(export_folder, optimisation_method, job_nmr, feed_option, crops, trade_scenario):
...
if __name__ == '__main__':
    ...
    argparser.add_argument('crops', help = "What crops to use as fodder feed: all 16 major crops ('all'), or the 9 main grains ('sub')")
    argparser.add_argument('trade_scenario', help = "Trade, feed constraints or no trade")
    ...
    parallelise(export_folder, optimisation_method, job_nmr, feed_option, crops, trade_scenario)