...
def scoring(feats, optimisation_method, crop_yield, lam, beef_yield, logger, feed_option):
    ...
    if feed_option in ['v1', 'v2']:
        ...
        # logger.info(feats.index.values)
        feats.loc[feats.ADM0_A3 == 'GRL'][['geometry', 'diff_potato']].to_file(export_folder + '/' + "grl1.gpkg", driver = 'GPKG')
        ...
        for l in ['cropland']:
            ...
            for f in crop_list:  # Loop though feeds
                ...
                if feed_option == "v1":
                    ...
                    qty_exported = ((feats['suitable_area'].values * feats[['ADM0_A3']].merge(
                        crop_area[['ADM0_A3', f + '_area']], how="left")[f + '_area'].values * \
                ...
                # trade partners
                trade_partners = feats[['ADM0_A3']].merge(feedpartners.loc[feedpartners.feed == f], how='left').drop(
                ...
                meat_abroad += np.nansum(qty_exported[:, None] * trade_partners * feed_energy[f].iloc[0] * partner_me[beef_yield].values[None, :],
                                                      axis=1) * dressing

                ### Methane emitted abroad (t CO2 eq) = Exported feed (t) * partner fraction (%) * methane emissions per biomass consumed (t/t)
                methane_abroad += np.nansum(qty_exported[:, None] * trade_partners *  partner_me["me_to_meth"].values[None, :], axis=1)

                ### N2O from manure emitted abroad (t CO2 eq) = Exported feed (t) * partner fraction (%) * N2O emissions per biomass consumed (t/t)
                manure_abroad += np.nansum(qty_exported[:, None] * trade_partners * partner_me["nitrous"].values[None, :], axis=1)
                ...
                exp_costs += np.nansum(qty_exported[:, None] * trade_partners * feats[['ADM0_A3']].merge(
                    expcosts.loc[expcosts.feed == f], how='left').drop(['ADM0_A3', 'feed'], axis=1).values, axis=1)
        ...
        logger.info(feats.index.values)
        feats.loc[feats.ADM0_A3 == 'GRL'][['geometry', 'diff_potato']].to_file(export_folder + '/' + "grl2.gpkg", driver = 'GPKG')
        ...
        feats.loc[feats.ADM0_A3 == 'GRL'][['geometry', 'diff_potato']].to_file(export_folder + '/' + "grl3.gpkg", driver = 'GPKG')
            ...
            for feed in ["grass", "grain", "stover"]:
                ...
                if feed == 'grass':
                    ...
                    for f in crop_list:
                        ...
                        trade_partners = feats[['ADM0_A3']].merge(feedpartners.loc[feedpartners.feed == f],
                                                                  how='left').drop(['ADM0_A3', 'feed'], axis=1).values

                        meat_abroad += np.nansum(qty_exported[:, None] * trade_partners * feed_energy[f].iloc[0] * partner_me[beef_yield].values[None, :], axis=1)
    ...
    ### Calculate score
    feats.loc[feats.ADM0_A3 == 'GRL'][['geometry', 'diff_potato']].to_file(export_folder + '/' + "grl4.gpkg", driver = 'GPKG')
    ...
    feats.loc[feats.ADM0_A3 == 'GRL'][['geometry', 'diff_potato']].to_file(export_folder + '/' + "grl5.gpkg", driver = 'GPKG')
    ...
    feats.loc[feats.ADM0_A3 == 'GRL'][['geometry', 'diff_potato']].to_file(export_folder + '/' + "grl6.gpkg", driver = 'GPKG')
    ...
    feats.loc[feats.ADM0_A3 == 'GRL'][['geometry', 'diff_potato']].to_file(export_folder + '/' + "grl7.gpkg", driver = 'GPKG')
...
def main(export_folder ='.', optimisation_method= 'weighted_sum', lam = 0.5, demand_scenario = 'Demand',
    ...
    grid.loc[grid.ADM0_A3 == 'GRL'][['geometry', 'diff_potato']].to_file(export_folder + '/' + "grl00.gpkg",
                                                                           driver='GPKG')
    ...
    grid.loc[grid.ADM0_A3 == 'GRL'][['geometry', 'diff_potato']].to_file(export_folder + '/' + "grl0.gpkg", driver = 'GPKG')
    ...
    grid.loc[grid.ADM0_A3 == 'GRL'][['geometry', 'diff_potato']].to_file(export_folder + '/' + "grl8.gpkg", driver = 'GPKG')
