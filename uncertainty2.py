...
def eac(cost, rate = 0.05, lifespan = 30.):
    ...
    rate (float)-> Discount rate, default = 5%
...
def scoring(feats, optimisation_method, crop_yield, lam, beef_yield, aff_scenario, logger, feed_option, landuses):
    ...
    opp_soc = feats['current_grazing'].fillna(0).values * feats['soil_carbon10km'].values * soc_change[
        ...
        'soil_carbon10km'].values * soc_change['cropland'].values * -1
    ...
    regrowth_area = np.where(
        (feats['current_grazing'].fillna(0).values + feats['current_cropping'].fillna(0).values) > feats[
            'available_area'].values, (feats['current_grazing'].fillna(0).values + feats[
            'current_cropping'].fillna(0).values) - feats['available_area'].values, 0)
    ...
    if feed_option in ['v1', 'v2']:
        ...
        for l in ['grass_grain']:
            ...
            grain_prod = grain_area[:, None] * feats[['ADM0_A3']].merge(foddercrop_area, how="left").drop(
                'ADM0_A3', axis=1).values * feats[['climate_bin']].merge(fodder_potential_yields, how="left").drop(
                'climate_bin', axis=1).values * feats[['ADM0_A3']].merge(fodder_yield_fraction, how="left").drop(
        ...
        for l in ['stover_grass']:
            ...
            stover_methane = np.nansum(stover_production, axis=1)* \
                                 feats[['group', 'glps']].merge(subset_table, how='left', left_on=['group', 'glps'],
                                                                right_on=['group', 'glps'])['curr_methane'].values
            ...
            stover_manure = np.nansum(stover_production, axis=1)* \
                                 feats[['group', 'glps']].merge(subset_table, how='left', left_on=['group', 'glps'],
                                                                right_on=['group', 'glps'])['curr_manure'].values
...
def main(export_folder ='.', optimisation_method= 'weighted_sum', lam = 0.5, demand_scenario = 'Demand',
    ...
    for i in new_colnames:
        grid[i] = np.take_along_axis(grid[[l + new_colnames[i] for l in landuses]].values,
