...
def scoring(feats, optimisation_method, crop_yield, lam, beef_yield, logger, feed_option):
    ...
    if feed_option in ['v1', 'v2']:
        ...
        for l in grass_cols:
            ...
            methane_table = energy_conversion.loc[energy_conversion.feed == 'grazing'][['region', 'climate', 'me_to_meth']]

            # Calculate methane production (ton CO2eq) = biomass consumed (t) * conversion factor (ton CO2eq/ton biomass)
            feats[l+'_meth'] = biomass_consumed * feats[['group', 'glps']].merge(methane_table, how='left', left_on = ['group', 'glps'],
            ...
            feats[l + '_cost'] = n_applied * 1.2 * fert_costs['n'].values + k_applied * fert_costs['k'].values + p_applied * \
                                 fert_costs['p'].values
        ...
        for l in ['cropland']:
            ...
            potential_prod = areas_cultivated * feats[['climate_bin']].merge(potential_yields, how="left").drop('climate_bin', axis=1).values
            ...
            if feed_option == 'v1':
                ...
                actual_prod = potential_prod * feats[['ADM0_A3']].merge(yield_fraction, how="left").drop('ADM0_A3', axis=1).values
            ...
            feats[l + '_area'] = np.nansum(actual_prod / (feats[['climate_bin']].merge(potential_yields, how="left").drop('climate_bin', axis=1).values * feats[
            ...
            methane_table = energy_conversion.loc[energy_conversion.feed == 'mixed'][['region', 'climate', 'me_to_meth']]
            ...
            for f in feedpartners['feed'].unique():  # Loop though feeds
                ...
                if feed_option == "v1":
                    ...
                    qty_exported = ((feats['suitable_area'].values * feats[['ADM0_A3']].merge(
                        crop_area[['ADM0_A3', f + '_area']], how="left").drop('ADM0_A3',axis=1)[f + '_area'].values * \
                               ...
                               feats[['ADM0_A3']].merge(yield_fraction, how="left").drop('ADM0_A3', axis=1)[f].values)) *\
                ...
                meat_abroad = meat_abroad + np.nansum(qty_exported[:, None] * \
                                                      ...
                                                      feed_energy[f].iloc[0] * partner_me[beef_yield].values[None, :], axis=1) * dressing
            ...
            feed_to_port_cost = ntrips_feed_exp * feats["distance_port"] * feats[['ADM0_A3']].merge(fuel_cost[['ADM0_A3', 'Diesel']],
                                                                       how = 'left',
                                                                       left_on = 'ADM0_A3',
            ...
            # Transport emissions: number of trips * emissions per trip (tons CO2 eq)
            beef_trans_emiss = ntrips_beef_mkt * feats["accessibility"] * fuel_efficiency * truck_emission_factor/1000.
        ...
        for l in ['stover']:
            ...
            stover_production = feats[['diff_' + i for i in crop_list]].values * crop_residues.iloc[0].values[None,:] * stover_availability
            ...
            meat_stover = stover_energy * feats[['group', 'glps']].merge(meat_table, how='left', left_on=['group', 'glps'],
                                                  right_on=['region', 'climate'])[beef_yield].values * dressing
            ...
            feats[l+'_trans_emiss'] = ntrips_beef_mkt * feats["accessibility"] * fuel_efficiency * truck_emission_factor/1000.
    ...
    ### If using v3 feed option, i.e., feed mix from PNAS
        ...
        for l in landuses:
            for feed in ["grass", "grain", "stover"]:
                ...
                if feed == 'grass':
                    # Calculate grazing biomass consumption
        ...
        for l in ['grazing', 'mixed']:
            ...
            feats[l + '_opp_cost'] = feats[[l + '_' + feed + '_opp_cost' for feed in ["grass", "grain", "stover"]]].sum(
                axis=1)
            feats[l + '_area'] = feats[[l + '_' + feed + '_area' for feed in ["grass", "grain", "stover"]]].sum(axis=1)
    ...
    opp_soc = feats['current_grazing'].fillna(0).values * feats['soil_carbon10km'].values * soc_change['grassland'].values * -1 + \
              feats['current_cropping'].fillna(0).values * feats['soil_carbon10km'].values * soc_change['cropland'].values * -1
...
def main(export_folder ='.', optimisation_method= 'weighted_sum', lam = 0.5, demand_scenario = 'Demand',
    ...
    cols = ['geometry','ADM0_A3', 'region', 'group','climate_bin', 'glps','suitable_area',  'best_score', 'bestlu', 'destination',
                                        'opp_cost', 'est_cost'] + list(new_colnames.keys())+  ["diff_" + crop for crop in crop_list]
