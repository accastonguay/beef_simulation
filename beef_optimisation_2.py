...
def scoring(feats, optimisation_method, crop_yield, lam, beef_yield, aff_scenario, logger, feed_option):
    ...
    if feed_option in ['v1', 'v2']:
        ...
        for l in grass_cols:
            ...
            meat_table = energy_conversion.loc[energy_conversion.feed == 'grazing'][['group', 'glps', beef_yield]]
        ...
        for l in ['grass_grain']:
            ...
            feats['grain_grassBM'] = grass_area * feats['grass_0500_N000'].values

            grain_prod = grain_area[:, None] * feats[['ADM0_A3']].merge(foddercrop_area, how="left").drop('ADM0_A3',
                                                                                                          axis=1).values * \
                         feats[['climate_bin']].merge(fodder_potential_yields, how="left").drop('climate_bin',
                                                                                                axis=1).values * feats[['ADM0_A3']].merge(
                fodder_yield_fraction, how="left").drop('ADM0_A3', axis=1).values
            ...
            biomass_dom = grain_prod * (
                    1 - feats[['ADM0_A3']].merge(percent_exported[['ADM0_A3'] + foddercrop_list], how="left").drop(
            ...
            meat_table = energy_conversion.loc[energy_conversion.feed == 'mixed'][['group', 'glps', beef_yield]]
            ...
            meat = (np.nansum(biomass_dom * feed_energy[foddercrop_list].iloc[0].values[None, :], axis=1) + (
                        feats['grain_grassBM'].values * feats.merge(grass_energy, how='left',
                                                                    left_on=['region', 'glps'],
                                                                    right_on=['region', 'glps'])['ME'].values)) * \
            ...
            feats[l + '_n2o'] = np.nansum(
                grain_area[:, None] * feats[['ADM0_A3']].merge(foddercrop_area, how="left").drop('ADM0_A3',
                                                                                                 axis=1).values * feats[
                    ['ADM0_A3']].merge(crop_emissions[['ADM0_A3'] + foddercrop_list], how="left").drop('ADM0_A3',
                                                                                                       axis=1).values,
                axis=1)
            ...
            for f in foddercrop_list:  # Loop though feeds
                ...
                if feed_option == "v2":
                    ...
                    qty_exported = ((grain_area *
                                     feats[['ADM0_A3']].merge(crop_area[['ADM0_A3', f + '_area']], how="left")[
                                         f + '_area'].values * \
                                     feats[['climate_bin']].merge(potential_yields[['climate_bin', f + '_potential']],
                                                                  how="left")[f + '_potential'].values * \
        ...
        for l in landuses:
            for feed in ["grass", "grain", "stover"]:
                ...
                feats[l + '_' + feed + '_area'] = feats['suitable_area'].values * feats[['region', 'glps']].merge(
                    ...
                    left_on=['region', 'glps'], right_on=['group', 'glps'])[feed].values
                ...
                if feed == 'grass':
                    ...
                    meat = np.nansum(biomass_dom * feed_energy.iloc[0].values[None, :], axis=1) * \
                           feats[['group', 'glps']].merge(me_table, how='left', left_on=['group', 'glps'],
                                                          right_on=['group', 'glps'])[beef_yield].values
                    local_meat = np.sum(np.where(feats[months] < -1,
                                                 (meat[:, None] - (
                                                         meat[:, None] * (-0.0182 * feats[months] - 0.0182))) / 12.,
...
def main(export_folder ='.', optimisation_method= 'weighted_sum', lam = 0.5, demand_scenario = 'Demand',
    ...
    # grid = pd.read_sql_query("SELECT * FROM grid WHERE ADM0_A3 ='BEL'", conn)
    # conn.close()
